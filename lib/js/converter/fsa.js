...
function FSA (states, alphabet, transitions, startState, acceptStates) {
    if (arguments.length !== 5) {
        // we will use this as a convenience for iterating through all the states in the machine
        this.states = []

        // changed from {} to []. We just need to be able to iterate through a list of possible symbols
        this.alphabet = []

        this.transitions = new Map()

        // a state ID should always be represented as a sorted array of integers
        this.startState = undefined

        // In accordance with the above definition, the set of acceptStates should always be a Set of sorted arrays of integers
        this.acceptStates = []
    } else {
        this.states = states
...
/*
   powerSet(states, incNull)

   *states*  List of strings which represent state IDs
   *incNull*  If true, the returned list includes an empty
     list as an element.
...
FSA.prototype.powerSet = function (states, incNull) {
    const powerset = states.slice(0) // start with the list of states
    let i, iter

    if (!states) return []
    if (arguments.length === 1) incNull = true
    ...
    while (iter < powerset.length) {
        ...
        for (i = 0; i < states.length; i++) {
            const psetList = powerset[iter].split(',')
            if (states[i] > psetList[psetList.length - 1]) {
                psetList.push(states[i])
                powerset.push(psetList.join(','))
            }
        }
        iter++
    }
    ...
    if (incNull) powerset.unshift('ES')
...
*/
FSA.prototype.epsilonClosure = function (states) {
    if (!(states instanceof Array)) {
        console.err('epsilonClosure called without an Array argument.')
        return false
    }

    const eclosedStates = new Set(states)
    const queue = new Set(states)
    const delta = this.transitions

    while (queue.size() !== 0) {
        /* add all states reachable on an epsilon transition to the queue
           and the list of eclosed states */
        const src = queue.data.shift()
        const dst = delta.find(src + '-E')
        if (dst) {
            for (let i = 0; i < dst.length; i++) {
                if (eclosedStates.insert(dst[i])) { queue.insert(dst[i]) }
            }
        }
    }
...
/**
  eclosedTransitions(states, symb)
...
FSA.prototype.eclosedTransitions = function (states, symb) {
    let i, j
    ...
    for (i = 0; i < states.length; i++) {
        transArray = this.transitions.find(states[i] + '-' + symb)
        if (transArray) {
            for (j = 0; j < transArray.length; j++) { transSet.insert(transArray[j]) }
        }
    }

    /* calculate the epsilon closure of these states */
    const postEclose = this.epsilonClosure(transSet.toArray()).toArray()
